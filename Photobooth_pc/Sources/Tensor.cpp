...
Tensor::Tensor(const Model& model, const TF_Output& oper)
{
	...
	{
		// Get dimensions
		_dims = new int64_t[n_dims];
		TF_GraphGetTensorShape(model.graph, this->op, _dims, n_dims, model.status);

		// Check error on Model Status
		model.status_check(true);

		this->shape = std::vector<int64_t>(_dims, _dims + n_dims);
...
void Tensor::clean() 
{
	...
	if (actual_shape != nullptr)
	{
		delete actual_shape;
		actual_shape = nullptr;
	}	
}

void  Tensor::error_check(bool condition, const QString& error)
{
	if (!condition) 
	{
		this->flag = -1;
...
void Tensor::set_data(std::vector<T>& new_data) 
{
	...
	auto exp_size = std::abs(std::accumulate(this->shape.begin(), this->shape.end(), int64_t(1), std::multiplies<int64_t>()));
	...
	this->val = TF_NewTensor(this->type, actual_shape->data(), (int)actual_shape->size(), this->data, sizeof(T) * new_data.size(), d, nullptr);
